...
function upComment(canvas) {
    ...
    var ballLeft = (comment.lastPoint.x + comment.Down.X) / 2 - 5;
    var ballTop = (comment.lastPoint.y + comment.Down.Y) / 2 - 5;
    $(commentBall)
        .addClass('commentNote')
        .css({
            'left': ballLeft + 'px',
...
function saveComment() {
    for (var i = 0; i < $('.commentObj').length; i++) {
        ...
        var temp = {
            ...
            left: $('#ball' + obj.id).offset().left / MainObj.Scale,
            top: $('#ball' + obj.id).offset().top / MainObj.Scale
        };
...
function reDrawComment(obj, callback) {
    ...
    var ballLeft = obj.left * MainObj.Scale;
    var ballTop = obj.top * MainObj.Scale;
